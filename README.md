...
当前版本优先保证本地 demo 闭环：不需要 API key，也能使用内置 fallback 引擎完成章节识别、场景拆分、角色摘要、冲突节奏和原文追溯。
...
- 创新点：角色关系摘要、场景冲突强度、原文追溯、改编风格选择、节奏统计、分场卡片预览。
...
4. 点击“生成结构化剧本 YAML”。
5. 在右侧查看并编辑 YAML，确认校验状态实时变化。
6. 查看页面底部结构化预览：角色关系、节奏指标和分场卡片。
7. 点击复制或下载 YAML。
8. 对照 [YAML Schema 文档](docs/yaml-schema.md) 解释字段设计。
9. 展示 [参考项目分析](docs/reference-analysis.md) 中的独立改写说明。
