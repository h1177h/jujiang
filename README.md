...
- 创新点：长篇逐章事件抽取、故事圣经合并、Schema 修复回合、单场 AI 补强、轻量版本历史、工作区草稿自动保存、章节事件图谱、场景级工作台编辑、章节到场景映射、冲突曲线、质量检查、角色关系摘要、原文追溯、改编风格选择、节奏统计、改编计划。
...
8. 在故事分析区点击章节事件、章节映射、冲突曲线或质量检查项，定位到对应场景。
9. 在场景编辑器里修改目标、地点、对白或冲突等级，确认右侧 YAML 和校验状态同步更新。
10. 有真实 API 时，点击“AI 补强”只重写当前场景，确认 YAML 和版本历史更新。
11. 刷新页面，确认原文、YAML、选中场景和版本历史仍在；再点击重置工作区，确认可以回到示例草稿。
12. 点击保存、恢复版本、复制或下载 YAML。
13. 对照 [YAML Schema 文档](docs/yaml-schema.md) 解释字段设计。
14. 展示 [参考项目分析](docs/reference-analysis.md) 中的独立改写说明。
