...
- 应用内 AI 服务会为模型调用创建本地生成任务，前端轮询任务状态；运行中可取消，失败后可复制诊断并直接重试，最近生成记录会随工作区草稿一起恢复。
...
src/
  ...
  core/
    ...
    generationRun.ts       # 生成任务阶段、取消、失败、完成和历史状态
...
5. 点击“生成结构化剧本 YAML”。
6. 观察生成计划和任务面板：长章节会先分片抽取，连接、事件抽取、故事圣经、剧本生成、Schema 修复和 YAML 写入都会留下状态；运行中可以取消，失败后可以复制诊断并直接重试。
