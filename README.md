...
- 自动识别章节并清洗文本。
- AI 生成采用分层流水线：短篇走事件图谱 + 剧本生成；长篇会逐章抽取事件，超长章节先按段落预算拆成片段，再合并故事圣经和改编策略，最后生成完整剧本 YAML。
- 原文区会显示生成计划：章节数、段落数、预计抽取单元和当前处理模式，长文生成不再是黑箱等待。
- 应用内 AI 服务会为模型调用创建本地生成任务，前端轮询任务状态；运行中可取消，刷新后会尝试恢复最近本地任务状态，失败后可复制包含 task id / request id 的诊断并直接重试。
...
- 工作区草稿会自动保存在本机浏览器：小说正文、标题、改编风格、YAML、选中场景、版本历史、本地任务快照和生成记录刷新后仍可恢复，也可以一键重置工作区。
...
- 创新点：生成计划预估、长章节分片抽取、本地任务化生成与取消、长篇逐章事件抽取、故事圣经合并、Schema 修复回合、生成任务面板、单场 AI 补强、轻量版本历史与版本对比、工作区草稿自动保存、章节事件图谱、场景级工作台编辑、章节到场景映射、冲突曲线、质量检查、角色关系摘要、原文追溯、改编风格选择、节奏统计、改编计划。
...
src/
  ...
  core/
    ...
    generationPlan.ts      # 长文生成计划、章节分片和抽取预算
    apiSettings.ts         # 浏览器端 AI 设置持久化
    workspaceDraft.ts      # 本机工作区草稿持久化
    generationRun.ts       # 生成任务阶段、任务快照、取消、失败、完成和历史状态
    ...
    __tests__/             # 核心测试
scripts/
  api-proxy.mjs             # 应用内 OpenAI-compatible AI 服务与本地生成任务运行时
docs/
...
5. 点击“生成结构化剧本 YAML”。
6. 观察生成计划和任务面板：长章节会先分片抽取，连接、事件抽取、故事圣经、剧本生成、Schema 修复、YAML 写入和本地任务快照都会留下状态；运行中可以取消，失败后可以复制诊断并直接重试。
