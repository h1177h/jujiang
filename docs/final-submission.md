...
7. 点击“生成结构化剧本 YAML”。
8. 观察生成任务面板，确认连接检查、事件抽取、故事圣经、剧本生成、Schema 修复和 YAML 写入都有阶段状态；失败后可以保留阶段信息并直接重试。
...
14. 刷新页面，确认标题、原文、YAML、选中场景、版本历史和最近生成记录会恢复；再点击重置工作区，回到示例草稿。
...
- 生成任务面板会记录每次调用的阶段、耗时、章节进度和失败位置，失败任务可以直接重试，长篇生成不再是黑盒等待。
- 支持单场 AI 补强，可以只重写当前场景的对白、冲突和场尾钩子，不必整篇重跑。
- 提供轻量版本历史，可保存、恢复和对比关键 YAML 快照。
- 可操作创新点：长篇逐章事件抽取、故事圣经合并、Schema 修复回合、生成任务面板、失败重试与生成记录恢复、单场 AI 补强、轻量版本历史、版本对比、工作区草稿自动保存、章节事件图谱、场景级工作台编辑、章节到场景映射、冲突曲线、质量检查、角色关系摘要、原文追溯、改编风格选择、节奏统计。
