...
11. 有真实 API 时，点击“AI 补强”局部重写当前场景，并确认版本历史新增快照。
12. 手动编辑右侧 YAML，观察 Schema 校验提示。
13. 刷新页面，确认标题、原文、YAML、选中场景和版本历史会恢复；再点击重置工作区，回到示例草稿。
14. 保存和恢复一个版本，再点击复制或下载 YAML。
15. 对照 `docs/yaml-schema.md` 讲字段设计。
16. 对照 `docs/reference-analysis.md` 讲参考项目借鉴和独立改写。
...
- 可操作创新点：长篇逐章事件抽取、故事圣经合并、Schema 修复回合、单场 AI 补强、轻量版本历史、工作区草稿自动保存、章节事件图谱、场景级工作台编辑、章节到场景映射、冲突曲线、质量检查、角色关系摘要、原文追溯、改编风格选择、节奏统计。
